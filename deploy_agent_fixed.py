#!/usr/bin/env python3
"""
Deploy the RAG Agent to Vertex AI Agent Engine (Full RAG Version).

This version includes full RAG functionality with proper corpus access.
"""
...
def create_full_rag_agent():
    """Create the full RAG agent with actual corpus functionality."""
    
    print("Creating full RAG agent for deployment...")
    
    from google.adk.agents import Agent
    from typing import List
    
    # Import the actual RAG functionality
    def rag_query_tool(query: str) -> dict:
        """Full RAG query tool with actual corpus access."""
        try:
            from vertexai import rag
            from rag_agent.config import DEFAULT_TOP_K, DEFAULT_DISTANCE_THRESHOLD
            
            # Use the hardcoded corpus resource name from your config
    ...
    def add_data_tool(paths: List[str]) -> dict:
        """Add data to the corpus with full functionality."""
        try:
            from vertexai import rag
...
"""
    ...
    def delete_document_tool(document_id: str) -> dict:
        """Delete document with full functionality."""
        try:
            from vertexai import rag
    ...
    # Create the full RAG agent
    full_rag_agent = Agent(
        name="HMKAgent_FullRAG",
        model="gemini-2.5-flash-preview-04-17",
        description="Vertex AI RAG Agent - Full RAG Functionality",
        tools=[
            rag_query_tool,
...
"""
    ...
    print("✓ Full RAG agent created with actual corpus functionality")
    return full_rag_agent

def deploy_to_agent_engine():
    """Deploy the full RAG agent to Vertex AI Agent Engine."""
    
    agent = create_full_rag_agent()
    
    print("\n" + "="*50)
    print("DEPLOYING FULL RAG AGENT TO VERTEX AI AGENT ENGINE")
    print("="*50)
    
    print("Starting deployment with full RAG functionality...")
    print("⚠️  This step may take several minutes to complete...")
    
    # Include full requirements for RAG functionality
    requirements = [
        "google-adk",
    ...
    try:
        ...
        print("✓ Full RAG Agent deployed successfully to Agent Engine!")
...
def test_full_rag_agent(remote_app):
    """Test the deployed agent with full RAG functionality."""
    print("\nTesting deployed agent with full RAG functionality...")
    
    # Create a remote session
    remote_session = remote_app.create_session(user_id="test_user_rag")
    print(f"✓ Remote session created: {remote_session['id']}")
    
    # Test queries on the remote agent
    test_queries = [
        "What is your corpus information?", 
        "Search for any project documents",
        "What construction documents do you have access to?",
        "Tell me about the project documentation available"
    ]
    ...
    print("✓ Full RAG testing completed")
    return remote_session

def main():
    """Main deployment workflow for full RAG functionality."""
    try:
        # Initialize Vertex AI
        initialize_vertex_ai()
        
        # Ask user if they want to proceed with full RAG deployment
        proceed = input("\nProceed with full RAG deployment to Agent Engine? (y/n): ")
        if proceed.lower() != 'y':
            print("Deployment cancelled by user.")
            return
        
        # Deploy to Agent Engine with full RAG functionality
        remote_app = deploy_to_agent_engine()
        
        # Test remote deployment
        print("\n" + "="*50)
        print("FULL RAG TESTING")
        print("="*50)
        remote_session = test_full_rag_agent(remote_app)
        
        # Success message
        print("\n" + "="*50)
        print("FULL RAG DEPLOYMENT SUCCESSFUL!")
        print("="*50)
        print(f"Your RAG agent with full functionality is now deployed to Vertex AI Agent Engine.")
        print(f"Resource name: {remote_app.resource_name}")
        print("\nFull RAG capabilities included:")
        print("✓ Real corpus querying with retrieval")
        print("✓ Document management (add/delete)")
        print("✓ Corpus information access")
        print("✓ Proper chunking and embedding")
        print("✓ LLM-based document parsing")
        print("\nYour agent can now:")
        print("- Search through your actual document corpus")
        print("- Add new documents with proper processing")
        print("- Delete documents when needed")
        print("- Provide detailed corpus information")
        print("\nTo interact with your deployed agent, use the chat interface:")
        ...
        print(f"❌ Error during full RAG deployment: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
